...
func (e eqCreateUserMatcher) String() string {
	return fmt.Sprintf("matches user %v", e.arg)
}
...
func TestCreateUser(t *testing.T) {
	...
	testCases := []struct {
		...
		checkStatus  func(recorder *httptest.ResponseRecorder)
	}{
		{
			name: "OK",
			responseBody: gin.H{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					CreateUser(gomock.Any(), EqCreateUser(arg, passwd)).
					Times(1).
					Return(user, nil)
			},
			checkStatus: func(recorder *httptest.ResponseRecorder) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			data, err := json.Marshal(tc.responseBody)
